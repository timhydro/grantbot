...
cat > "$CANONICAL" <<'GRANTBOT_V17_PAYLOAD'
#!/usr/bin/env bash
set -Eeuo pipefail
...
printf '\n============================================================\n'
printf ' GRANTBOT PRO V17 — ALL-IN-ONE INSTALLER\n'
printf '============================================================\n'
printf 'Installer: %s\n' "$CANONICAL"
printf 'Log:       %s\n\n' "$LOG"

if ! bash -n "$CANONICAL"; then
  printf 'ERROR: Embedded V17 installer failed Bash syntax validation.\n' | tee -a "$LOG"
  printf 'Terminal remains open.\n'
