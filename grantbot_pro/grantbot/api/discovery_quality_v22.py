...
from grantbot.discovery.rerank_v22 import rerank_payload
from grantbot.pipeline.live_queue_v16 import (
    DEFAULT_KEYWORDS,
    discover_ranked_queue,
    health as legacy_health,
    load_latest,
    load_run,
    promote_opportunity,
)
...
router = APIRouter(
    prefix="/v22/queue",
    tags=["GrantBot Discovery Quality v22"],
)
...
def queue_health() -> dict[str, Any]:
    return {
        "ranking_version": "22.0",
        "legacy_queue": legacy_health(),
    }
...
def latest() -> dict[str, Any]:
    try:
        return rerank_payload(load_latest())
...
def run(run_id: str) -> dict[str, Any]:
    try:
        return rerank_payload(load_run(run_id))
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
